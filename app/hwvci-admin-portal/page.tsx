...
export default function AdminPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      setMessage({
        type: "error",
        text: `Failed to ${editingEvent ? "update" : "create"} event: ${error.message}`,
      });
    } else {
      setMessage({
        type: "success",
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-accent/20 via-background to-background">
        <div className="bg-card p-8 rounded-2xl shadow-2xl border border-accent/20 w-full max-w-md animate-fadeInUp">
          ...
          {message && (
            <div
              className={`mb-4 p-3 rounded-lg flex items-center gap-2 animate-slideIn ${message.type === "error" ? "bg-red-500/10 text-red-500" : "bg-green-500/10 text-green-500"}`}
            >
              {message.type === "error" ? (
          ...
          <input
            ...
            className="w-full px-4 py-3 rounded-xl border border-border bg-background text-foreground font-light focus:outline-none focus:ring-2 focus:ring-accent transition-all duration-200 mb-4"
          />

          <button
            onClick={handleLogin}
            className="w-full bg-accent text-accent-foreground py-3 rounded-xl font-medium hover:opacity-90 hover:scale-[1.02] transition-all duration-200"
          >
            Login
          </button>
        </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-accent/5 via-background to-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 animate-fadeInUp">
          <div>
            <h1 className="text-3xl sm:text-4xl font-light tracking-tight bg-gradient-to-r from-foreground to-accent bg-clip-text text-transparent">
              Event Manager
            </h1>
          ...
          <div className="flex gap-3">
            <button
              ...
              className="bg-accent text-accent-foreground px-4 py-2 rounded-xl font-medium flex items-center gap-2 hover:opacity-90 hover:scale-[1.02] transition-all duration-200"
            >
            ...
            <button
              ...
              className="bg-red-500/10 text-red-500 px-4 py-2 rounded-xl font-medium hover:bg-red-500/20 transition-all duration-200"
            >
              Logout
            </button>
          </div>
        ...
        {message && (
          <div
            className={`mb-6 p-4 rounded-xl flex items-center gap-3 animate-slideIn ${message.type === "error" ? "bg-red-500/10 text-red-500" : "bg-green-500/10 text-green-500"}`}
          >
            {message.type === "error" ? (
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-5">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2 rounded-lg border border-border bg-background text-foreground font-light focus:outline-none focus:ring-2 focus:ring-accent transition-all duration-200"
                ...
                <div className="flex gap-3 pt-4">
                  <button
                    type="submit"
                    disabled={isLoading}
                    className="flex-1 bg-accent text-accent-foreground py-2 rounded-xl font-medium hover:opacity-90 hover:scale-[1.02] transition-all duration-200 disabled:opacity-50 disabled:scale-100 flex items-center justify-center gap-2"
                  >
                    {isLoading ? (
                  ...
                  <button
                    ...
                    className="flex-1 bg-muted text-foreground py-2 rounded-xl font-medium hover:bg-muted/80 transition-all duration-200"
                  >
                    Cancel
                  </button>
                </div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 flex-wrap mb-2">
                        <span
                          className="text-xs font-medium px-2 py-1 rounded-full transition-all duration-200 group-hover:scale-105"
                          style={{
                      ...
                      <h3 className="text-lg font-medium tracking-tight mb-2 group-hover:text-accent transition-colors duration-200">
                        {event.title}
                      </h3>
